...
if (Test-Path $SkillsDir) {
    ...
    if (Test-Path (Join-Path $SkillsDir ".git")) {
        ...
        Write-Host "⚠️ Existing skills directory is not a git repository. Skipping git pull." -ForegroundColor Yellow
    }
} else {
...
if (Test-Path $TemplatePath) {
    if (-not (Test-Path $LocalAgentsPath)) {
        ...
        Write-Host "ℹ️ Project AGENTS.md already exists. Skipping overwrite." -ForegroundColor Yellow
    }
}
