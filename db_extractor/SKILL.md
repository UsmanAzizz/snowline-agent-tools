...
3. **Security Note**: This tool only extracts schema metadata (tables, columns, types) and does not read row data, so passwords or sensitive tokens stored in the database are never leaked. Note that if the database connection fails, the native connection error message may include the database `username` (e.g. `Access denied for user...`), but the connection password remains perfectly masked.
